...
const loginUser = async (req, res) => {
  ...
  try {
      ...
      const existingTokenIndex = user.tokens.findIndex((t) => {
          try {
              const decoded = jwt.verify(t.token, process.env.JWT_SECRET);
              return t.deviceId === deviceId && decoded.userId === user._id.toString() && decoded.exp > now;
          } catch (err) {
              return false;
          }
      });
      ...
      if (existingTokenIndex > -1) {
          return res.status(409).json({
              error: "User is already logged in on this device.",
          });
      }

      user.tokens = user.tokens.filter((t) => !(t.deviceId === deviceId && jwt.decode(t.token)?.userId === user._id.toString()));
