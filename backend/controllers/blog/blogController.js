...
const createBlog = async (req, res) => {
    try {
        ...
        if (!["Student", "Tutor"].includes(formattedUploaderType)) {
            console.error("Lỗi: Uploader type không hợp lệ:", formattedUploaderType);
            return res.status(400).json({ error: "Uploader type không hợp lệ" });
        }

        const uploaderModel = formattedUploaderType === "Tutor" ? Tutor : Student;
        ...
        const newBlog = new Blog({
            ...
            content,
            uploaderId,
            uploaderType: formattedUploaderType, // Lưu đúng định dạng enum
            tags: tags || [],
            imageUrl: imageUrl || "",
            likes: 0,
        });
