...
export const createCommentDocument = async (documentId, commentData) => {
    try {
        ...
        const response = await axios.post(
            ...
            { ...commentData, documentId, authorId: userId, authorType: formattedAuthorType },
...
export const getCommentsByDocument = async (documentId) => {
    try {
        const response = await axios.get(`${API_URL}/api/commentdocument/document/${documentId}`);
        return response.data;
    } catch (error) {
...
export const editCommentDocument = async (commentId, updatedContent) => {
    try {
        ...
        console.error("Lỗi khi chỉnh sửa bình luận:", error.response?.data || error.message);
        throw error;
    }
};
...
export const deleteCommentDocument = async (commentId) => {
    try {
        ...
        const response = await axios.delete(`${API_URL}/api/commentdocument/document/${commentId}`, {
            headers: { Authorization: `Bearer ${token}` },
        });
        return response.data;
    } catch (error) {
