...
import { getConversations, getConversationById, getAllUser, createConversations, sendMessage } from "../../../../api_service/mesages_service";
...
export default function MessagePage() {
  ...
  const [isAddConversationModalOpen, setIsAddConversationModalOpen] = useState(false); // Modal thêm cuộc trò chuyện
              ...
              data.map(async (conversation) => {
                ...
                const updatedParticipants = await Promise.all(
                  conversation.participants.map(async (participant) => {
                    const userDetails = await getUserDetails(participant.participantId);
                    return {
                      ...participant,
    ...
    socketIo.on("receiveMessage", (message) => {
      if (selectedConversation === message.conversationId && !sendingMessage) {
        setMessages((prevMessages) => {
          ...
          if (!prevMessages.some((msg) => msg.content === message.content && msg.timestamp === message.timestamp)) {
            return [...prevMessages, message].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)); // Sort by timestamp
          }
          return prevMessages;
        });
      }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await sendMessage(newMsg.conversationId, newMsg.senderId, newMsg.senderModel, newMsg.content);
      setMessages((prevMessages) => {
        const updatedMessages = [...prevMessages, newMsg].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
  ...
  return (
    <Content style={stylesInline.content}>
      <Flex gap="middle">
            ...
            <Flex gap="small" vertical>
              {filteredData.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <Card.Meta
                      avatar={<Avatar src={`https://api.dicebear.com/7.x/miniavs/svg?seed=${otherParticipant.participantId}`} />}
        ...
        <div style={{ height: "60vh", overflowY: "auto", padding: "1rem" }} ref={messagesContainerRef}>
  {messages.length === 0 ? (
    <p>No messages yet</p>
  ) : (
    messages.map((msg, index) => (
      <div
        ...
        style={{
          ...
          alignItems: "center",
          marginBottom: "1rem",
          justifyContent: msg.senderId === userId ? "flex-end" : "flex-start",  // Thay đổi vị trí căn của người gửi và người nhận
        }}
      >
        {msg.senderId !== userId && (
        ...
        <div>
          ...
          <div
            style={{
              ...
              padding: "0.5rem 1rem",
              borderRadius: "10px",
              background: msg.senderId === userId ? "#dcf8c6" : "#ffffff",
